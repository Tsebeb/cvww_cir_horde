...
class HordeModel(CIL_Net):
    def __init__(self, network_name, pretrained, remove_existing_head,
        ...
        super(HordeModel, self).__init__(network_name, pretrained, remove_existing_head)
        self.number_of_feature_extractors = num_fe
    ...
    def modify_head(self, num_outputs):
        if self.linear is None:
            if self.hidden_dim_size <= 0:
                self.linear = self.head_class(self.out_size * self.number_of_feature_extractors, num_outputs)
            else:
                layers = [Linear(self.out_size * self.number_of_feature_extractors, self.hidden_dim_size)]
            ...
            if self.hidden_dim_size <= 0:
                new_linear = self.head_class(self.out_size * self.number_of_feature_extractors, num_outputs)
    ...
    def forward(self, x: torch.Tensor, return_features=False):
        features = torch.randn((x.size(0), self.number_of_feature_extractors * self.out_size), device=x.device)
        features = features * 0.01
        for m, model in enumerate(self.feature_extractors):
            features[:, m * self.out_size: (m + 1) * self.out_size] = model(x)
