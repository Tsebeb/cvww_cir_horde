...
class Scenario_Loader(Manage_Loaders):
    ...
    def __init__(self, datasets, validation, num_tasks, batch_size, num_workers, pin_memory, imgs_per_task,
                 num_start_classes, num_start_samples, reoccurrence_prob_min, reoccurrence_prob_max):
        self.num_start_classes = num_start_classes
        self.num_start_samples = num_start_samples
        self.imgs_per_task = imgs_per_task
        self.reoccurrence_prob_min = reoccurrence_prob_min
        self.reoccurrence_prob_max = reoccurrence_prob_max
        assert 0 <= self.reoccurrence_prob_min < self.reoccurrence_prob_max <= 1
    ...
    def extra_parser(args):
        ...
        parser.add_argument("--reoccurrence-prob-min", type=float, default=0.05)
        parser.add_argument("--reoccurrence-prob-max", type=float, default=0.75)
        return parser.parse_known_args(args)

    def _generate_tasks(self):
        rand_table = np.random.uniform(size=(self.num_classes, self.num_tasks))
        class_reoccurrence_prop = np.random.uniform(low=self.reoccurrence_prob_min,
                                                    high=self.reoccurrence_prob_max,
