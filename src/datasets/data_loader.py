...
def get_datasets(dataset, path, num_tasks, nc_first_task, validation, trn_transform, tst_transform, class_order=None):
    ...
    if 'mnist' in dataset:
        ...
        all_data, taskcla, class_indices = memd.get_data(trn_data, tst_data, validation=validation,
                                                         ...
                                                         shuffle_classes=False, class_order=class_order)
        # set dataset type
...
def get_transforms(resize, pad, crop, flip, normalize, extend_channel, auto_augment, brightness):
    ...
    if brightness is not None:
        trn_transform_list.append(transforms.ColorJitter(brightness=brightness))

    # AutoAugment
    if auto_augment is not None:
        trn_transform_list.append(AutoAugment(policy=auto_augment))
