...
dataset_config = {
    ...
    "cifar100_autoaugment": {
        'path': join(_BASE_DATA_PATH, 'cifar100'),
        'normalize': ((0.5071, 0.4866, 0.4409), (0.2009, 0.1984, 0.2023)),
        'flip': True,
        "auto_augment": AutoAugmentPolicy.CIFAR10
    },
    'cifar100_icarl': {
        'path': join(_BASE_DATA_PATH, 'cifar100'),
        'resize': None,
    ...
    'tiny_imagenet_autoaugment': {
        'path': join(_BASE_DATA_PATH, 'tiny_imagenet_200'),
        'resize': None,
        'normalize': ((0.481, 0.457, 0.408), (0.260, 0.253, 0.268)),
        "auto_augment": AutoAugmentPolicy.IMAGENET
    }
}

# Add missing keys:
for dset in dataset_config.keys():
    for k in ['resize', 'pad', 'crop', 'normalize', 'class_order', 'extend_channel', "auto_augment", "brightness"]:
