...
from utils import _get_all_targets, _get_unique_targets
...
class ExemplarsSelector:
    ...
    def __call__(self, model: LLL_Net, trn_loader: DataLoader, transform):
        clock0 = time.time()
        exemplars_per_class = self._exemplars_per_class_num(trn_loader)
...
class RandomExemplarsSelector(ExemplarsSelector):
    ...
    def _select_indices(self, model: LLL_Net, sel_loader: DataLoader, exemplars_per_class: int, transform) -> Iterable:
        result = []
        labels = _get_all_targets(sel_loader.dataset)
        unique_clases = torch.unique(labels)
        for curr_cls in unique_clases:
            ...
            if exemplars_per_class <= len(cls_ind):
                ...
                warnings.warn("Less samples than requested are available sampling what is possible ", )
...
def dataset_transforms(dataset, transform_to_change):
    if isinstance(dataset, ConcatDataset):
        ...
        return dataset_transforms(dataset.dataset, transform_to_change)
    else:
